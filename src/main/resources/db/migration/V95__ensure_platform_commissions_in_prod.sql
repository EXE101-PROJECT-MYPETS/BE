...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1
                       ...
                       WHERE t.typname = 'commission_source_type'
                         AND n.nspname = 'prod') THEN
            CREATE TYPE prod.commission_source_type AS ENUM ('ORDER', 'SERVICE_BOOKING', 'VET_BOOKING');
        END IF;
        ...
        IF NOT EXISTS (SELECT 1
                       ...
                       WHERE t.typname = 'commission_status'
                         AND n.nspname = 'prod') THEN
            CREATE TYPE prod.commission_status AS ENUM ('PENDING', 'INVOICED', 'COLLECTED', 'REFUNDED', 'CANCELED');
        END IF;

        IF NOT EXISTS (SELECT 1
                       FROM pg_type t
                                JOIN pg_namespace n ON n.oid = t.typnamespace
                       WHERE t.typname = 'commission_invoice_status'
                         AND n.nspname = 'prod') THEN
            CREATE TYPE prod.commission_invoice_status AS ENUM ('PENDING', 'PAID', 'OVERDUE', 'CANCELED');
        END IF;
    END
$$;

CREATE TABLE IF NOT EXISTS prod.platform_commissions
(
    id                  bigserial PRIMARY KEY,
    shop_id             bigint                      NOT NULL REFERENCES prod.shops (id) ON DELETE CASCADE,
    source_type         prod.commission_source_type NOT NULL,
    ...
    status              prod.commission_status      NOT NULL DEFAULT 'PENDING',
...
CREATE INDEX IF NOT EXISTS idx_platform_commissions_shop_status_created
    ON prod.platform_commissions (shop_id, status, created_at);

CREATE INDEX IF NOT EXISTS idx_platform_commissions_status_created
    ON prod.platform_commissions (status, created_at);

CREATE TABLE IF NOT EXISTS prod.platform_commission_invoices
(
    id                      bigserial PRIMARY KEY,
    shop_id                 bigint                         NOT NULL REFERENCES prod.shops (id) ON DELETE CASCADE,
    ...
    status                  prod.commission_invoice_status NOT NULL DEFAULT 'PENDING',
...
CREATE INDEX IF NOT EXISTS idx_platform_commission_invoices_shop_status_due
    ON prod.platform_commission_invoices (shop_id, status, due_at);

CREATE INDEX IF NOT EXISTS idx_platform_commission_invoices_status_due
    ON prod.platform_commission_invoices (status, due_at);

CREATE TABLE IF NOT EXISTS prod.platform_commission_invoice_items
(
    id                bigserial PRIMARY KEY,
    invoice_id        bigint      NOT NULL REFERENCES prod.platform_commission_invoices (id) ON DELETE CASCADE,
    commission_id     bigint      NOT NULL REFERENCES prod.platform_commissions (id),
    commission_amount bigint      NOT NULL DEFAULT 0,
    created_at        timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS idx_platform_commission_invoice_items_invoice
    ON prod.platform_commission_invoice_items (invoice_id);
